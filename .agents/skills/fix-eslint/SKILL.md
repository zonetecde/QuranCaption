...
name: fix-eslint
description:
  Automatically fix ESLint errors by modifying code to comply with linting rules. For small
  codebases (≤20 errors), fixes directly. For larger codebases (>20 errors), spawns parallel agents
  per directory for efficient processing. Never disables rules or adds ignore comments.
...
**If ≤20 errors:** Proceed to Step 3 (Direct Fix) **If >20 errors:** Proceed to Step 4 (Parallel
Fix)
...
```markdown
Task 1 (src/auth/): prompt: "Fix all ESLint errors in the src/auth/ directory.
...
subagent_type: "general-purpose" description: "Fix auth directory ESLint errors"
```

```markdown
Task 2 (src/api/): prompt: "Fix all ESLint errors in the src/api/ directory.
...
subagent_type: "general-purpose" description: "Fix API directory ESLint errors"
```
