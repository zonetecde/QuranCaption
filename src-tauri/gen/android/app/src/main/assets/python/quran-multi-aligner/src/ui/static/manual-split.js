...
var MANUAL_SPLIT_MIN_TEXT_HEIGHT = Number(window.MANUAL_SPLIT_TEXT_MIN_HEIGHT_PX) || 96;
var MANUAL_SPLIT_MAX_TEXT_HEIGHT = Number(window.MANUAL_SPLIT_TEXT_MAX_HEIGHT_PX) || 420;
...
function resetManualSplitPending(card) {
	...
	confirmBtn.disabled =
		!card._manualSplitBoundaryStarts || card._manualSplitBoundaryStarts.size === 0;
}
...
function exitManualSplit(card) {
	...
	if (
		textDiv &&
		typeof card._manualSplitOriginalHtml === 'string' &&
		!isStaleManualSplitCard(card)
	) {
...
function submitManualSplit(card) {
	...
	var cuts = Array.from(boundaries)
		.sort(function (a, b) {
			return a - b;
		})
		.map(function (startIdx) {
			return startIdx - 1;
		});
