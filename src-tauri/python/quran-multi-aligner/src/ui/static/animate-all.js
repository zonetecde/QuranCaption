		...
		childNodes.forEach(function (node) {
			if (
				...
				if (renderedPositions[pos]) {
					...
					if (posParts.length >= 1 && posParts[0]) {
						...
						if (wordSurah !== lastSurah) {
							...
							if (
								trailingSpecial &&
								trailingSpecial.classList &&
								trailingSpecial.classList.contains('mega-special-line')
							) {
				...
				if (!markerText.trim() || !renderedMarkers[markerKey]) {
					...
					var lastWordPos =
						segWordEls.length > 0 ? segWordEls[segWordEls.length - 1].dataset.pos || '' : '';
				...
				if (txt.trim()) {
					...
					if (!renderedMarkers[markerKey]) {
						...
						if (posPartsM2.length >= 2)
							markerSpan2.dataset.verse = posPartsM2[0] + ':' + posPartsM2[1];
...
function startSegmentTick(seg, audio, segStartTime, segEndTime) {
	...
	animDebug(
		'MEGA',
		'startSegmentTick seg=' +
			segIdx +
			' cacheWords=' +
			seg.cacheWords.length +
			' cacheChars=' +
			seg.cacheChars.length +
			' bounds=[' +
			segStartTime.toFixed(3) +
			',' +
			segEndTime.toFixed(3) +
			']'
	);
	function tick() {
		...
		if (window.ANIM_GRANULARITY !== lastGranularity) {
			animDebug(
				'MEGA',
				'Granularity changed: ' + lastGranularity + ' -> ' + window.ANIM_GRANULARITY
			);
		...
		if (window.ANIM_OPACITY_PREV !== lastOpacityPrev) {
			animDebug(
				'MEGA',
				'Mode changed: prevOp ' + lastOpacityPrev + ' -> ' + window.ANIM_OPACITY_PREV
			);
			animateAllState.unifiedCacheWords.forEach(function (c) {
		...
		if (window._windowSettingsVersion !== lastSeenVersion) {
			if (lastWordIdx >= 0) {
				var wc =
					window.ANIM_GRANULARITY === 'Characters'
						? animateAllState.unifiedCacheChars
						: animateAllState.unifiedCacheWords;
				var map =
					window.ANIM_GRANULARITY === 'Characters' ? seg.unifiedCharMap : seg.unifiedWordMap;
		...
		if (
			lastWordIdx >= 0 &&
			lastWordIdx < cache.length &&
			currentTime >= cache[lastWordIdx].start &&
			currentTime < cache[lastWordIdx].end
		) {
			newWordIdx = lastWordIdx;
			searchPath = 'same';
		} else if (
			lastWordIdx + 1 < cache.length &&
			currentTime >= cache[lastWordIdx + 1].start &&
			currentTime < cache[lastWordIdx + 1].end
		) {
		...
		if (newWordIdx !== lastWordIdx) {
			...
			if (newWordIdx >= 0) {
				...
				var map =
					window.ANIM_GRANULARITY === 'Characters' ? seg.unifiedCharMap : seg.unifiedWordMap;
				applyWindowOpacity(wc, map[newWordIdx], lastWordIdx >= 0 ? map[lastWordIdx] : -1);
			}
...
function onAnimateAllSegmentEnd() {
	...
	if (nextIdx < animateAllState.segments.length) {
		...
		nextSeg.cacheWords.forEach(function (c) {
			c.el.dataset.segIdx = nextIdx;
		});
		nextSeg.cacheChars.forEach(function (c) {
			c.el.dataset.segIdx = nextIdx;
		});
...
function stopAnimateAll() {
	...
	// Unhide group wrappers (split / missing-words)
	document
		.querySelectorAll('.split-group.hidden-for-mega, .missing-words-group.hidden-for-mega')
		.forEach(function (g) {
			g.classList.remove('hidden-for-mega');
		});
	...
	if (animAccordion && animateAllState.accordionParent) {
		if (animateAllState.accordionNextSibling) {
			animateAllState.accordionParent.insertBefore(
				animAccordion,
				animateAllState.accordionNextSibling
			);
		} else {
		...
		if (wantOpen !== isOpen) {
			var accBtn = animAccordion.querySelector(':scope > .label-wrap');
...
function toggleAnimateAll(btn) {
	...
	// Hide group wrappers (split / missing-words) — otherwise their dashed
	// border + tag render as empty bordered shells behind the mega card.
	document
		.querySelectorAll('.segments-container .split-group, .segments-container .missing-words-group')
		.forEach(function (g) {
			g.classList.add('hidden-for-mega');
		});
	...
	tip.textContent =
		'Tip: Click on any word to seek to it, or click a verse marker to jump to the start of that verse.';
...
document.addEventListener('click', function (e) {
	...
	if (e.target.closest('.repeat-fb-up')) {
		...
		if (group) {
			...
			group.querySelectorAll('.repeat-fb-btn').forEach(function (b) {
				b.remove();
			});
	...
	if (e.target.closest('.ref-editable') && !e.target.closest('.ref-edit-input')) {
		...
		var refCard = span.closest('.segment-card');
		if (refCard && refCard.classList.contains('manual-split-mode')) {
			return;
		}
...
(function () {
	...
	function applyEditPatch(patch) {
		...
		if (patch.status === 'error') {
			...
			if (container && patch.edited_idx != null && patch.old_ref) {
				var card = container.querySelector(
					'.segment-card[data-segment-idx="' + patch.edited_idx + '"]'
				);
				if (card) {
		...
		if (patch.matched_text_html) {
			var edited = container.querySelector(
				'.segment-card[data-segment-idx="' + patch.edited_idx + '"]'
			);
			if (edited) {
	...
	function setupPatchObserver() {
		...
		if (!patchEl) {
			setTimeout(setupPatchObserver, 500);
			return;
		}
		...
		var observer = new MutationObserver(function () {
			...
			try {
				...
				console.error('[EDIT-PATCH] error:', e);
			}
