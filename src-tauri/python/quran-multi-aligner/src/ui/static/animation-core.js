...
function saveAnimSettings() {
	try {
		...
		s.color =
			getComputedStyle(document.documentElement).getPropertyValue('--anim-word-color').trim() ||
			window.ANIM_WORD_COLOR_DEFAULT;
		// Read text styling from slider DOM values (more reliable than mega-card inline styles)
		var wsEl = document.querySelector(
			'#anim-word-spacing input[type=range],#anim-word-spacing input[type=number]'
		);
		var tsEl = document.querySelector(
			'#anim-text-size input[type=range],#anim-text-size input[type=number]'
		);
		var lsEl = document.querySelector(
			'#anim-line-spacing input[type=range],#anim-line-spacing input[type=number]'
		);
...
if (_saved) {
	...
	window.ANIM_OPACITY_PREV = _initPreset
		? _initPreset.prev_opacity
		: window.ANIM_OPACITY_PREV_DEFAULT;
	window.ANIM_OPACITY_AFTER = _initPreset
		? _initPreset.after_opacity
		: window.ANIM_OPACITY_AFTER_DEFAULT;
	window.ANIM_WINDOW_PREV = _initPreset ? _initPreset.prev_words : window.ANIM_WINDOW_PREV_DEFAULT;
	window.ANIM_WINDOW_AFTER = _initPreset
		? _initPreset.after_words
		: window.ANIM_WINDOW_AFTER_DEFAULT;
...
function ensureVerseIndex(cache) {
	...
	animDebug(
		...
		'ensureVerseIndex: ' +
			cache.length +
			' elements, ' +
			Object.keys(verseRanges).length +
			' verses'
	);
}
...
function applyWindowOpacity(cache, newIdx, prevIdx) {
	animDebug(
		'WINDOW',
		'applyWindowOpacity newIdx=' + newIdx + ' prevIdx=' + prevIdx + ' cacheLen=' + cache.length
	);
	...
	if (
		...
		pcAll === oldPcAll &&
		acAll === oldAcAll &&
		pc === window._windowLastPc &&
		ac === window._windowLastAc
	) {
		if (pcAll) {
...
function updateWindowMaxLabel(elemId, val, maxVal) {
	...
	if (!el.querySelector('.max-hint')) {
		...
		if (rangeWrap) {
			...
			hint.style.cssText =
				'position: absolute; right: 0; bottom: -1.2em; font-size: 0.7em; opacity: 0.5;';
...
function clearHighlights(card) {
	card
		.querySelectorAll('.word.active, .word.reached, .char.active, .char.reached')
		.forEach(function (w) {
...
function startAnimation(audio, card) {
	...
	function tick() {
		...
		if (
			lastWordIdx >= 0 &&
			lastWordIdx < wordCache.length &&
			currentTime >= wordCache[lastWordIdx].start &&
			currentTime < wordCache[lastWordIdx].end
		) {
			newWordIdx = lastWordIdx;
			searchPath = 'same';
		} else if (
			lastWordIdx + 1 < wordCache.length &&
			currentTime >= wordCache[lastWordIdx + 1].start &&
			currentTime < wordCache[lastWordIdx + 1].end
		) {
			...
			if (
				newWordIdx === -1 &&
				wordCache.length > 0 &&
				currentTime >= wordCache[wordCache.length - 1].start
			) {
...
function toggleAnimation(btn) {
	...
	if (isActive) {
		...
		animDebug(
			'START',
			'toggleAnimation: words=' +
				audio._cacheWords.length +
				' chars=' +
				audio._cacheChars.length +
				' granularity=' +
				window.ANIM_GRANULARITY
		);
