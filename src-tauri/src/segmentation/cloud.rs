...
fn prepared_audio(
    ...
    let source = if let Some(clips) = audio_clips.as_ref().filter(|v| !v.is_empty()) {
        let (path, guard) = merge_audio_clips_for_segmentation(clips, &temp_dir)?;
    ...
    let stamp = SystemTime::now()
        ...
        .map_err(|e| e.to_string())?
        .as_millis();
    let output_path = temp_dir.join(format!("qurancaption-mfa-{}.wav", stamp));
    ...
    if let Some((start, _)) = window {
        args.extend(["-ss".to_string(), format!("{:.3}", start as f64 / 1000.0)]);
    }
...
pub async fn segment_quran_audio(
    ...
    let stamp = SystemTime::now()
        ...
        .map_err(|e| e.to_string())?
        .as_millis();
    let path = temp_dir.join(format!("qurancaption-seg-{}.ogg", stamp));
