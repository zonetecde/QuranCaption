<script lang="ts">
	...
	const isLegacyLocal = $derived(() => wizard.selection.aiVersion === 'legacy_v1' && wizard.selection.mode === 'local');
	const isMultiLocal = $derived(() => wizard.selection.aiVersion === 'multi_v2' && wizard.selection.mode === 'local');
	const asrLabel = $derived(() => (wizard.selection.aiVersion === 'legacy_v1' ? 'Legacy Whisper' : 'Multi-Aligner'));
</script>
...
<section class="space-y-4">
	...
	{#if wizard.selection.mode === 'local'}
		<div class="inline-flex items-center gap-2 rounded-full border border-color bg-accent/70 px-3 py-1 text-xs text-secondary">
	...
	{#if wizard.selection.mode === 'api'}
		<div class="grid grid-cols-1 gap-2 xl:grid-cols-2">
			{#each MULTI_MODEL_OPTIONS as option}
				<button type="button" class="rounded-lg border p-3 text-left" class:border-accent-primary={wizard.selection.cloudModel === option.value} class:border-color={wizard.selection.cloudModel !== option.value} onclick={() => wizard.setCloudModel(option.value)}>
	...
	{:else if isLegacyLocal()}
		<div class="space-y-2">
			{#each LEGACY_MODEL_OPTIONS as option}
				<button type="button" class="w-full rounded-lg border p-3 text-left" class:border-accent-primary={wizard.selection.legacyModel === option.value} class:border-color={wizard.selection.legacyModel !== option.value} onclick={() => wizard.setLegacyModel(option.value)}>
					<div class="text-sm font-medium text-primary">{option.label}</div>
					<div class="text-xs text-thirdly">{option.description}</div>
				</button>
			{/each}
	...
	{:else if isMultiLocal()}
		<div class="grid grid-cols-1 gap-2 xl:grid-cols-2">
			{#each MULTI_MODEL_OPTIONS as option}
				<button type="button" class="rounded-lg border p-3 text-left" class:border-accent-primary={wizard.selection.multiModel === option.value} class:border-color={wizard.selection.multiModel !== option.value} onclick={() => wizard.setMultiModel(option.value)}>
					<div class="text-sm font-medium text-primary">{option.label}</div>
					<div class="text-xs text-thirdly">{option.description}</div>
				</button>
			{/each}
	...
	{#if isMultiLocal()}
		<div class="rounded-xl border border-color bg-accent/70 p-3">
			...
			<div class="flex gap-2">
				<button class="btn-accent px-3 py-1.5 text-xs" onclick={() => void wizard.promptHFToken()}>{wizard.selection.hfToken ? 'Update token' : 'Set token'}</button>
				<button class="btn px-3 py-1.5 text-xs" onclick={() => void wizard.clearHFToken()} disabled={!wizard.selection.hfToken}>Clear token</button>
			</div>
		</div>
	{/if}
