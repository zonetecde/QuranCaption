<script lang="ts">
	...
	function applyResizeValue(styleId: StyleName, value: number): void {
		...
		const time = Math.max(0, Math.floor(globalState.getTimelineState.cursorPosition));
		const hasKeyframes =
			typeof styles.getKeyframeTimes === 'function' &&
			styles.getKeyframeTimes(styleId, clipIds).length > 0;
		if (hasKeyframes && typeof styles.setKeyframe === 'function') {
			styles.setKeyframe(styleId, time, constrainedValue, clipIds);
		} else if (clipIds.length > 0 && typeof styles.setStyleForClips === 'function') {
			styles.setStyleForClips(clipIds, styleId, constrainedValue);
		} else {
			style.value = constrainedValue;
		}
	}
