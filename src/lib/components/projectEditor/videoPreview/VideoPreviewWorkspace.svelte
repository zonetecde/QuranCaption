<script lang="ts">
	...
	import AntiCollisionNotice from './AntiCollisionNotice.svelte';
	import PreviewBackgroundMedia from './PreviewBackgroundMedia.svelte';
	...
	let showAntiCollisionNotice = $derived(
		!isExportCapturePreview &&
			...
			Boolean(globalState.getStyleValue('global', 'anti-collision'))
	);
	...
	$effect(() => {
		...
		untrack(() => {
			...
			applyAudioVolume(currentClip instanceof AssetClip ? currentClip.volumePercent : 100);
		});
	});
	 ...
	 * @returns {number} Temps en secondes dans l'audio.
	 */
	 ...
	 * @returns {number} Temps en secondes dans la vidéo.
	 */
	...
	/**
	 * Synchronise la vitesse de lecture des sources audio et vidéo actives.
	 * @param {number} speed Multiplicateur de vitesse à appliquer.
	 * @returns {void}
	 */
	...
	/**
	 * Résout la vitesse de lecture adaptée au mode d'édition courant.
	 ...
	 * @param {number} volumePercent Volume du clip demandé entre 0 et 100.
	 * @returns {number} Volume natif entre 0 et 1.
	 */
		...
		for (const clip of activeClips) {
			...
			if (!howl) {
				howl = new Howl({
					...
					volume: Math.min(1, Math.max(0, clip.volumePercent / 100))
				});
	...
	async function playNativeAudioAtCursor(): Promise<void> {
		...
		try {
			const status = await controlNativeAudio('play', {
				...
				volume: getNativeAudioVolume(
					currentAudioClip instanceof AssetClip ? currentAudioClip.volumePercent : 100
				),
				speed: audioSpeed
			});
	...
	/**
	 * Bascule la prévisualisation entre lecture et pause.
	 * @returns {void}
	 */
	...
	function applyAudioVolume(volumePercent: number): void {
		...
		for (const [clipId, howl] of overlappingAudioHowls) {
			...
			howl.volume(
				clip instanceof AssetClip ? Math.min(1, Math.max(0, clip.volumePercent / 100)) : 1
			);
		}
		if (nativeAudioReady) {
	...
	function setupHowlerFallback(audioAsset: Asset): Howl {
		...
		const currentAudioClip = globalState.getAudioTrack.getCurrentClip();
		applyAudioVolume(currentAudioClip instanceof AssetClip ? currentAudioClip.volumePercent : 100);
		return audioHowl;
	}
	 ...
	 * @param {boolean} _fromButton Indique si l'action vient du bouton play.
	 * @returns {void}
	 */
	 ...
	 * @param {number} val Position en secondes.
	 * @returns {void}
	 */
	 ...
	 * @param {unknown} error Erreur renvoyée par le lecteur natif ou le navigateur.
	 * @returns {boolean} `true` lorsque l'erreur provient du verrouillage autoplay.
	 */
	...
	>
		...
		<div class="relative origin-top-left overflow-hidden bg-black" id="preview">
			{#if showAntiCollisionNotice}
				<AntiCollisionNotice
					copy={antiCollisionNoticeCopy}
					onDismiss={dismissAntiCollisionNotice}
				/>
			{/if}
			{#if !globalState.getVideoPreviewState.showVideosAndAudios}
				...
				<PreviewBackgroundMedia
					...
					opacity={transitionState.currentOpacity}
					showCrossfadeNotice={transitionState.showCrossfadeNotice}
					crossfadeNotice={getCrossfadePreviewNotice()}
					onVideoEnded={goNextVideo}
				/>
			{/if}
