<script lang="ts">
	...
	let settingsSections = $derived([
		...
		{ name: $LL.settings.backup(), tab: SettingsTab.BACKUP, icon: 'archive' },
		{ name: $LL.settings.contact(), tab: SettingsTab.CONTACT, icon: 'mail' },
		{ name: $LL.settings.about(), tab: SettingsTab.ABOUT, icon: 'info' }
	]);
...
>
	<!-- Header -->
	<div class="bg-secondary px-4 pt-5 pb-3 border-b border-color flex items-center gap-3">
		{#if settingsMenuOpen}
			<div
				class="w-10 h-10 shrink-0 bg-accent-primary rounded-full flex items-center justify-center shadow-md"
			>
				<span class="material-icons text-black text-xl">settings</span>
			</div>
		{:else}
			<button
				class="w-10 h-10 shrink-0 rounded-full hover:bg-accent flex items-center justify-center text-secondary hover:text-primary"
				onclick={() => (settingsMenuOpen = true)}
		...
		<div class="min-w-0 flex-1">
			<h2 class="text-lg font-semibold text-primary truncate">{$LL.settings.settings()}</h2>
		</div>

		<!-- Close btn -->
		<button
			class="w-10 h-10 shrink-0 rounded-full hover:bg-accent flex items-center justify-center transition-all duration-200 text-secondary hover:text-primary"
	...
	<div class="flex-1 min-h-0">
		{#if settingsMenuOpen}
			<div class="bg-primary h-full p-3 overflow-auto flex flex-col">
				<div class="flex flex-col gap-1">
					{#each settingsSections as setting (setting.tab)}
						<button
							class="flex items-center gap-3 min-h-12 px-3 py-2 rounded-xl w-full transition-colors duration-150 text-left"
							class:selected={globalState.uiState.settingsTab === setting.tab}
							onclick={() => {
								globalState.uiState.settingsTab = setting.tab;
								settingsMenuOpen = false;
							}}
						>
							<span class="material-icons text-accent-secondary">{setting.icon}</span>
							<span class="flex-1 truncate text-sm">{setting.name}</span>
							<span class="material-icons text-base text-thirdly">chevron_right</span>
						</button>
					{/each}
				</div>
