...
/**
 * Retourne le clip Quran actuellement edite en mode WBW manuel.
 *
...
/**
 * Quitte le mode d'edition WBW manuel et reinitialise son etat runtime.
 *
 * @returns {void}
 */
 ...
 *
 * @param {SubtitleClip} clip Clip Quran a editer.
 * @returns {Promise<boolean>} `true` si le mode a ete ouvert.
 */
...
/**
 * Restaure le zoom timeline memorise avant l'entree en mode WBW.
 *
 * @returns {void}
 */
...
/**
 * Deplace la selection du mot courant dans la session WBW.
 *
...
/**
 * Deplace la borne partagee entre deux mots du brouillon WBW.
 *
 ...
 * @returns {void}
 */
export function updateManualWordByWordBoundary(boundaryIndex: number, newBoundaryTimeS: number): void {
	const clip = getActiveManualWordByWordClip();
	const wbwEdit = globalState.shared.wbwEdit;
	...
	const maxTimeS =
		nextWord.end > nextWord.start ? nextWord.end : Math.max(minTimeS, clipDurationS);
	...
	applyManualWordByWordDraftUpdate(clip, normalizeManualWordByWordDraftWords(draftWords, clipDurationS));
}

/**
 * Deplace le debut du mot courant sur la position du curseur timeline.
 *
 * @returns {void}
 */
...
/**
 * Deplace la fin du mot courant sur la position du curseur timeline.
 *
 * @returns {void}
 */
...
/**
 * Valide le mot courant a la position du curseur timeline.
 *
 * @returns {void}
 */
 ...
 *
 * @param {SubtitleClip} clip Clip Quran a transformer en brouillon editable.
 * @returns {Promise<ManualWordByWordDraftWord[]>} Liste des mots a editer.
 */
...
/**
 * Retourne la duree d'un clip Quran en secondes.
 *
 * @param {SubtitleClip} clip Clip cible.
 * @returns {number} Duree positive du clip.
 */
...
/**
 * Recale le selecteur de mots sur le mot WBW actuellement actif.
 *
 * @param {SubtitleClip} clip Clip Quran cible.
 * @returns {void}
 */
...
/**
 * Normalise un brouillon WBW pour garder une chaine continue et non decroissante.
 *
 * @param {ManualWordByWordDraftWord[]} draftWords Brouillon source.
 * @param {number} clipDurationS Duree du clip, en secondes.
 * @returns {ManualWordByWordDraftWord[]} Brouillon nettoye.
 */
function normalizeManualWordByWordDraftWords(
	...
	return draftWords.map((word, index) => {
		...
		const end = isLastWord
			? clipDurationS
			: clampWordTime(word.end, start, clipDurationS);
		previousEnd = end;
		return {
			...word,
			start,
...
/**
 * Repercute le brouillon WBW courant sur le clip cible et sur l'etat runtime.
 *
 * @param {SubtitleClip} clip Clip Quran cible.
 * @param {ManualWordByWordDraftWord[]} draftWords Brouillon a appliquer.
 * @returns {void}
 */
 ...
 *
 * @param {SubtitleClip} clip Clip Quran cible.
 * @param {ManualWordByWordDraftWord[]} draftWords Brouillon a persister.
 * @returns {void}
 */
function syncManualWordByWordDraftToClip(
	...
	clip.alignmentMetadata =
		buildSubtitleAlignmentMetadata(clip.alignmentMetadata?.source ?? 'local', segment, draftWords) ?? {
 ...
 *
 * @param {SubtitleClip} clip Clip Quran edite manuellement.
 * @returns {void}
 */
...
/**
 * Definit la fin du dernier mot du brouillon WBW.
 *
 ...
 * @param {number} minTimeS Minimum autorise.
 * @param {number} maxTimeS Maximum autorise.
 * @returns {number} Temps borne.
 */
