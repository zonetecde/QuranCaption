...
export interface FetchTranslationResult {
	fetched: number;
	review: TranslationReviewCounts;
}

export interface TranslationReviewCounts {
	total: number;
	complete: number;
	pending: number;
	fetched: number;
 ...
 * @returns {TranslationReviewCounts} Compteurs de revue de l'édition.
 */
export function getProjectTranslationReviewCounts(
	project: Project,
	editionName: string
): TranslationReviewCounts {
	const review: TranslationReviewCounts = {
		total: 0,
		complete: 0,
		pending: 0,
		fetched: 0,
