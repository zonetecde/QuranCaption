<script lang="ts">
	...
	import { ExportMediaInputBuilder } from './ExportMediaInputBuilder';
	import { ExportOverlayInspector } from './ExportOverlayInspector';
	...
	/**
	 * Retourne la configuration des fondus audio et vidéo utilisée par l'export.
	 * @returns {ExportFadeSettings} Configuration active des fondus.
	 */
	 ...
	 * @param {number} progress Progression brute à contraindre.
	 ...
	 * @param {ExportState} state État d'export à examiner.
	 * @returns {boolean} `true` pour un état de traitement des sous-titres.
	 */
	...
	/**
	 * Synchronise l'interface avec un événement de progression provenant de l'export natif.
	 * @param {ExportProgressEvent} event Événement de progression reçu depuis Tauri.
	 * @returns {Promise<void>} Promesse résolue après la mise à jour de l'état d'export.
	 */
	...
	/**
	 * Finalise l'état de progression lorsque l'export natif se termine.
	 * @param {ExportCompleteEvent} event Événement contenant le fichier exporté.
	 * @returns {Promise<void>} Promesse résolue après la notification de fin.
	 */
	...
	/**
	 * Transmet à l'interface une erreur émise par l'export natif courant.
	 * @param {ExportErrorEvent} event Événement d'erreur reçu depuis Tauri.
	 * @returns {Promise<void>} Promesse résolue après la notification de l'erreur.
	 */
	...
	/**
	 * Enrichit puis transmet la progression à la fenêtre principale.
	 * @param {ExportProgress} progress Progression primaire de l'export.
	 * @returns {Promise<void>} Promesse résolue après l'émission de l'événement.
	 */
	...
	/**
	 * Sélectionne le pipeline adapté puis démarre l'export demandé.
	 * @returns {Promise<void>} Promesse résolue lorsque le pipeline a été lancé ou terminé.
	 */
	...
	/**
	 * Résout le flou de l'overlay actif à un instant de la timeline.
	 * @param {number} time Position à inspecter en millisecondes.
	 * @returns {number} Intensité du flou applicable.
	 */
	...
	/**
	 * Collecte les frontières de clips susceptibles de modifier le flou vidéo.
	 * @param {number} rangeStart Début de la plage en millisecondes.
	 * @param {number} rangeEnd Fin de la plage en millisecondes.
	 * @returns {number[]} Frontières comprises dans la plage.
	 */
	...
	/**
	 * Découpe une plage en segments dont le niveau de flou reste constant.
	 * @param {number} rangeStart Début de la plage en millisecondes.
	 * @param {number} rangeEnd Fin de la plage en millisecondes.
	 * @returns {BlurSegment[]} Segments prêts à être rendus séparément.
	 */
	...
	/**
	 * Rend chaque segment de flou séparément puis assemble les vidéos obtenues.
	 * @param {BlurSegment[]} renderSegments Segments à capturer et encoder.
	 * @param {number} totalDuration Durée totale exportée en millisecondes.
	 * @returns {Promise<void>} Promesse résolue après l'assemblage des segments.
	 */
	...
	/**
	 * Crée le dossier temporaire qui recevra les captures d'un segment.
	 * @param {string} segmentImageFolder Nom relatif du dossier du segment.
	 * @returns {Promise<void>} Promesse résolue lorsque le dossier existe.
	 */
	...
	/**
	 * Indique si un instant doit réutiliser une capture sans sous-titre visible.
	 * @param {number} timing Instant inspecté en millisecondes.
	 * @param {Record<string, number[]>} blankImgs Timings regroupés par image vide.
	 * @param {Record<string, number>} imgWithNothingShown Timings sans aucun overlay affiché.
	 * @returns {boolean} `true` lorsqu'une capture vide doit être utilisée.
	 */
	...
	function buildImageCapturePlan(
		...
		return buildExportCaptureJobPlan({
			...
			getReusableBlankFileName: (timing) =>
				timings.styleKeyframeTimings.has(timing)
					? null
					: ExportOverlayInspector.getReusableBlankFileName(
	 ...
	 * @returns {Promise<number[]>} Index des captures vides produites.
	 */
	...
	/**
	 * Encode un segment vidéo à partir de ses captures et de ses médias.
	 * @param {number} segmentIndex Index du segment dans le pipeline.
	 * @param {string} segmentImageFolder Dossier relatif des captures.
	 * @param {number} segmentStart Début absolu du segment en millisecondes.
	 * @param {number} segmentDuration Durée du segment en millisecondes.
	 * @param {number} blur Intensité de flou appliquée au fond.
	 * @param {number[]} blankTimings Index des captures vides réutilisables.
	 * @returns {Promise<string>} Chemin du fichier vidéo temporaire généré.
	 */
	async function generateVideoForSegment(
		...
		const { audios, audioClips } = ExportMediaInputBuilder.getAudioInputs();
		const videos = ExportMediaInputBuilder.getVideoInputs();
	...
	/**
	 * Assemble les vidéos temporaires dans le fichier d'export final.
	 * @param {string[]} videoFilePaths Chemins des segments à concaténer dans l'ordre.
	 * @returns {Promise<void>} Promesse résolue après l'assemblage ou la notification d'erreur.
	 */
	...
	/**
	 * Exécute le pipeline d'export continu lorsqu'aucun découpage par flou n'est requis.
	 * @param {number} exportStart Début absolu de l'export en millisecondes.
	 * @param {number} exportEnd Fin absolue de l'export en millisecondes.
	 * @param {number} totalDuration Durée totale exportée en millisecondes.
	 * @param {number} blur Intensité de flou appliquée au fond.
	 * @returns {Promise<void>} Promesse résolue après la capture et l'encodage.
	 */
	...
	/**
	 * Calcule le plan de captures nécessaire pour une plage d'export.
	 * @param {number} rangeStart Début absolu de la plage en millisecondes.
	 * @param {number} rangeEnd Fin absolue de la plage en millisecondes.
	 * @returns {ExportCaptureTimingResult} Timings uniques et métadonnées de réutilisation.
	 */
	...
	/**
	 * Lance l'encodage natif d'un export vidéo continu.
	 * @param {number} exportStart Début absolu de l'export en millisecondes.
	 * @param {number} duration Durée à encoder en millisecondes.
	 * @param {number} blur Intensité de flou appliquée au fond.
	 * @param {number[]} blankTimings Index des captures vides réutilisables.
	 * @returns {Promise<void>} Promesse résolue après le lancement de l'encodage.
	 */
	...
	/**
	 * Ferme les workers de capture et supprime les fichiers temporaires de l'export.
	...
	/**
	 * Capture l'overlay courant dans un fichier PNG prêt pour FFmpeg.
	 * @param {string} fileName Nom du fichier sans extension.
	 * @param {string | null} subfolder Sous-dossier temporaire facultatif.
	 * @param {string | null} reusableBlankFileName Capture vide pouvant être réutilisée.
	 * @param {boolean} hideArabicText Masque les glyphes arabes pendant la capture.
	 * @returns {Promise<void>} Promesse résolue une fois l'image écrite.
	 */
	async function takeScreenshot(
		...
		if (isBlankScreenshot) {
			const keepSubtitleBackgrounds = ExportOverlayInspector.hasVisibleSubtitleBackground(node);
	 ...
	 * @param {string | number} sourceFileName Le nom du fichier source (sans extension).
	 * @param {number} targetFileName Le nom du fichier cible (sans extension).
	 * @param {string | null} subfolder Le sous-dossier où se trouvent les fichiers.
	 * @returns {Promise<void>} Promesse résolue lorsque la copie est terminée.
	 */
	 ...
	 * @returns {Promise<void>} Promesse résolue après la tentative de suppression.
	 */
